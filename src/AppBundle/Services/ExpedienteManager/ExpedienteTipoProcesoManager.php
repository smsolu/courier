...
class ExpedienteTipoProcesoManager extends Manager{
    ...
    public function getList(Expediente $expediente, $order_col, $order_status, $page = 1, $resultpage= 10, $filtro = array()){
        try{
           $queryBuilder = $this->em->getRepository('AppBundle:Expediente')->getTipoProcesodeExpediente($this->estudio,$expediente);
            $list = new ListView();
            $list
                ->setTitle("Otros Procesos Asociados")
                ->addColumn('', 'id',array(
                    'type' => 'link',
                ...
                ->addColumn('', 'id',array(
                    'type' => 'link',
                    'route' => new LinkColumn('expediente_tipoproceso_delete', array('idExpedienteTipoProc'=> 'getId','id'=>'getExpedienteid')),
                    'value' => '<span style="color:blue" class="glyphicon glyphicon-play-circle"></span>',
                ))                   
                ->addColumn("Procesos", 'NombreProceso',array(
                    'type' => 'string',
                    'allow_order'=>'0',
                    ))
                ->setPage($page)
